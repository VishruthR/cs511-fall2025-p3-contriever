...
if __name__ == "__main__":
    ...
    NQ_passages = {}
    for split in ['test']:
        with open('./scripts/evaluation/FiD/open_domain_data/download/NQ.' + split + '.idx.json', 'r') as fin:
            NQ_idx[split] = json.load(fin)
        with open('./scripts/evaluation/FiD/open_domain_data/download/nq_passages/' + (split + '.json'), 'r') as fin:
            NQ_passages[split] = json.load(fin)

    originaldev = []
    with open('./scripts/evaluation/FiD/open_domain_data/download/NQ-open.dev.jsonl') as fin:
